...
//			$con->exec("set names utf8");
	...
	public static function call($ps,$param=null)
	{
		...
		return $row;
		*/
	}public static function none_call($ps,$param=null)
	{
		$con=self::connection_open();
...
//			echo $cmd ; exit;
	...
	public static function insertinto_bullk($table,$col,$param)
	{
		$result="";
		$con=self::connection_open();
		try
	...
	public static function insertinto_bullk_ON_DUPLICATE($table,$col,$param)
	{
		$result="";
		$con=self::connection_open();
		try
		{
			...
			for($i=0; $i< count($arr_col); $i++){
				$updateCol.= ($i==0? "":",") . $arr_col[$i]." = VALUES(" . $arr_col[$i].")";
			}
			$sql="INSERT INTO $table ($col)VALUES $param ON DUPLICATE KEY UPDATE $updateCol";//age = VALUES(age),";
//			echo $sql ; exit;
			$con->exec($sql);
			$result = $con->lastInsertId();
		}
	...
	public static function execute_non_qury_no_resualt($cmd)
	{
		$con=self::connection_open();
